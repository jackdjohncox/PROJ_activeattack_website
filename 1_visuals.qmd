...
**Attacker Age**  
While the median age of attackers was `{r} age_median`, attackers ranged in age 
...
(`{r} outdoors_pct`%) happened in public (outdoor) spaces. For instance, on 
...
Schools were attacked `{r} school_n` times (`{r} school_pct`%). These events 
are aggregated and include attacks on all types of educational facilities, 
including K-12 and institutes of higher education. Lastly, `{r} other_n` 
attacks (`{r} other_pct`%) happened at "Other" locations (e.g., houses of 
worship).
...
these, `{r} format_num(kill_tot)` people died. As can be seen here, the number 
