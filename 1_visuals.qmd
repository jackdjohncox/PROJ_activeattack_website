...
# other %
other_pct <- get_pct(aa_data, relationship, "Other")

# student n
student_n <- get_count(aa_data, relationship, "Student")


#student attacks on schools
...
In a majority of attacks (`{r} norel_pct`%), the attacker(s) had no prior 
association with the attack location. Despite businesses being the most common 
attack location, only `{r} coworker_pct`% of attackers were carried out by 
current/former employees. While only `{r} student_n` attacks were carried out 
by current/former students, these attacks accounted for `{r} school_student_pct` 
attacks on schools. The final grouping included all remaining relationship 
types (`{r} other_pct`% of attacks), some of which were obscure (e.g., former 
patient) or were "indirect", that is, their spouse or other family member had 
the primary association with the attack location.

**Relationship by Location Type**  
When looking within location types, we find that most attackers had no 
relationship to the locations. However, some interesting patterns appear for 
cases with some kind of relationship. For instance, attacks on schools were 
rarely carried out by current/former employees or by individuals with no 
relationship to the school (`{r} school_coworker_pct`% and 
`{r} school_norel_pct`% of school attacks, respectively). Meaning that 
`{r} round(school_student_pct+school_other_pct,1)`% of school attacks are 
committed by someone who is associated with the school. 
...
occur during the morning ours prior to midday. Note: this plot shows the 
...
#get overall totals
injur_tot <- get_total(aa_data, c(wound_tot,kill_tot))
...
There were `{r} format_num(injur_tot)` casualties of active attacks between 
...
**Attacks & Fatalities by Year** (Interactive)  
